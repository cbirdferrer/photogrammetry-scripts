...
class App(QWidget):
    ...
    def initUI(self):
        ...
        items = ('yes','no')
        volchoice, okPressed = QInputDialog.getItem(self, 'Do you want body volume to be calculated? (you have to have measured Total Length widths)','',items,0,False)
        ...
        if safety == 'yes':
            if option == 'Individual Folders':
                ...
                for f in csvs:
                    ...
                    idx = df0.loc[df0[0]=='Object Name'].index #find the row number of 'Object Name'
                    df = pd.read_csv(f,sep= ',', header = idx[0]) #use row num to skip the right number of rows
                    #print(df)
                    widths = df.columns.values.tolist()

                    l = df['Object Name'].tolist() #make a list of all measurement names in this column
            ...
            if option == 'Individual Folders':
                ...
                for f in (f for f in files if f.endswith('.csv')):
                    ...
                    if anydup(l) == True: #check for any duplicate measurement names, if exists, exit code, print error msg
                        ...
                        print(f)
                        for i in l: #loop through list of measurement types
                            for w in (w for w in widths if w[0].isdigit() and w != 'Object Name'): #loop through the widths
                                x = df.loc[i,w] #extract cell value of width of measurement type
                                if pd.isna(x) == False: #if the cell isn't empty
                                    ww = i + "_" + w #combine the names
                                    measurements += [ww] #add this combined name to the master list
